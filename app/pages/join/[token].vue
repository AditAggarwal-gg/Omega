<template>
  <section class="max-w-md mx-auto px-6 py-20 text-center">
    ...
    <template v-else-if="preview">
      <p class="badge mb-4 mx-auto w-fit">
        INVITE
      </p>
      ...
      <p
        v-if="preview.is_expired"
        class="text-danger-500"
      >
        This invite has expired.
      </p>
      <p
        v-else-if="preview.is_accepted"
        class="text-paper-400"
      >
      ...
      <template v-else>
        ...
        <div v-else-if="!currentUser">
          <p class="text-paper-400 text-sm mb-4">
            Sign in or create an account with {{ preview.invited_email }} to accept.
          </p>
          <div class="flex gap-3 justify-center">
            <NuxtLink
              :to="`/login?next=/join/${token}`"
              class="rounded-md border border-ink-600 px-4 py-2 hover:border-signal-500"
            >Sign in</NuxtLink>
            <NuxtLink
              :to="`/signup?next=/join/${token}`"
              class="rounded-md bg-signal-500 px-4 py-2 text-ink-950 font-medium hover:bg-signal-400"
        ...
        <div v-else>
          ...
          <button
            :disabled="accepting"
            class="rounded-md bg-signal-500 px-5 py-2.5 font-medium text-ink-950 hover:bg-signal-400 disabled:opacity-50"
            @click="accept"
          >
