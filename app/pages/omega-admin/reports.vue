<template>
  <section class="max-w-4xl mx-auto px-6 py-12">
    ...
    <h1 class="text-3xl mb-8">All reports</h1>
    ...
    <div v-else-if="filtered.length" class="card divide-y divide-ink-700">
      <div v-for="r in filtered" :key="r.id" class="p-4">
        <div class="flex items-center justify-between mb-2">
          <div class="flex items-center gap-2">
            <span class="badge">{{ r.reason }}</span>
            <span v-if="!r.org_id" class="badge border-signal-500 text-signal-500">no org · flagged link</span>
          </div>
          <select v-model="r.status" class="bg-ink-900 border border-ink-600 rounded-md px-2 py-1 text-sm capitalize"
        ...
        <p v-if="r.details" class="text-sm text-paper-200 mb-2">{{ r.details }}</p>
        <p v-if="r.flagged_url" class="text-xs text-paper-400 font-mono break-all mb-1">{{ r.flagged_url }}</p>
        <p v-if="r.content_id" class="text-xs text-paper-400">Content ID: {{ r.content_id }}</p>
        <p class="text-xs text-paper-400 mt-2">{{ new Date(r.created_at).toLocaleString() }}</p>
      </div>
    </div>
    <div v-else class="card p-8 text-center text-paper-400">No {{ activeStatus }} reports.</div>
  </section>
</template>
