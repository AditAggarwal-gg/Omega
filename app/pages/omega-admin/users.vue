<template>
  <section class="max-w-3xl mx-auto px-6 py-12">
    ...
    <div v-else class="card divide-y divide-ink-700">
      <div v-for="p in filtered" :key="p.id" class="p-4 flex items-center justify-between">
        <div>
          <p class="font-medium">{{ p.full_name || 'Unnamed' }}</p>
          <p v-if="p.is_platform_admin" class="text-xs text-signal-500 font-mono">platform admin</p>
        </div>
        <button class="text-sm text-paper-400 hover:text-signal-500" @click="toggleAdmin(p)">
          {{ p.is_platform_admin ? 'Revoke admin' : 'Make admin' }}
        </button>
      </div>
      <p v-if="!filtered.length" class="p-4 text-paper-400 text-sm">No users found.</p>
    </div>
  </section>
</template>
