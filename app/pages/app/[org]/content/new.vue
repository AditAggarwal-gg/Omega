<template>
  <section class="max-w-2xl mx-auto px-6 py-12">
    <p class="badge mb-2">NEW CONTENT</p>
    <h1 class="text-3xl mb-8">{{ membership?.organizations.name }}</h1>

    <form class="card p-6 space-y-4" @submit.prevent="save">
      <div>
        <label class="block text-sm text-paper-200 mb-1" for="type">Type</label>
        <select id="type" v-model="type" class="w-full rounded-md bg-ink-900 border border-ink-600 px-3 py-2">
      ...
      <div>
        ...
        <input id="slug" v-model="slug" type="text" required pattern="[a-z0-9-]+"
      ...
      <div>
        ...
        <textarea id="summary" v-model="summary" rows="2"
      ...
      <div v-if="type === 'article'">
        ...
        <textarea id="body" v-model="body" rows="10"
          class="w-full rounded-md bg-ink-900 border border-ink-600 px-3 py-2 text-sm font-mono" />
      </div>

      <div v-else>
        <label class="block text-sm text-paper-200 mb-1" for="media">{{ type === 'video' ? 'Video' : 'Audio' }} URL</label>
        <input id="media" v-model="mediaUrl" type="url" required placeholder="https://…"
          class="w-full rounded-md bg-ink-900 border border-ink-600 px-3 py-2 text-sm" />
        <p class="text-xs text-paper-400 mt-1">Link to a hosted file (S3, Supabase Storage, YouTube direct file, etc).</p>
      </div>
      ...
      <div class="flex gap-3">
        <button type="submit" :disabled="saving" class="rounded-md bg-signal-500 px-4 py-2.5 font-medium text-ink-950 hover:bg-signal-400 disabled:opacity-50"
          @click="publishNow = false">
          {{ saving ? 'Saving…' : 'Save as draft' }}
        </button>
        <button type="submit" :disabled="saving" class="rounded-md border border-signal-500 px-4 py-2.5 font-medium text-signal-500 hover:bg-signal-500/10 disabled:opacity-50"
