<template>
  <div class="fixed inset-0 bg-ink-950/80 flex items-center justify-center z-50 px-4" @click.self="$emit('close')">
    <div class="card p-6 w-full max-w-sm">
      <h3 class="text-lg mb-4">Report this content</h3>

      <div v-if="checkingSession" class="text-paper-400 text-sm">Checking your session…</div>

      <div v-else-if="!currentUser" class="text-paper-200 text-sm">
        <p class="mb-4">Sign in to submit a report.</p>
        <NuxtLink to="/login" class="text-signal-500 hover:underline">Sign in</NuxtLink>
      </div>

      <form v-else-if="!submitted" class="space-y-4" @submit.prevent="submit">
        <div>
        ...
        <div>
          <label class="block text-sm text-paper-200 mb-1" for="details">Details (optional)</label>
        ...
        <div class="flex gap-2 justify-end">
          <button type="button" class="px-4 py-2 text-sm text-paper-400 hover:text-paper-50" @click="$emit('close')">Cancel</button>
      ...
      <div v-else class="text-paper-200 text-sm">
        <p class="mb-4">Thanks — your report has been submitted for review.</p>
        <button class="rounded-md border border-ink-600 px-4 py-2 text-sm hover:border-signal-500" @click="$emit('close')">Close</button>
      </div>
    </div>
  </div>
</template>
